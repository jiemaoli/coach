...
**文档目标**: 将 `ninetrans_book.pdf` 的知识点重构为可浏览、可学习、可检索、可考核的静态知识网站，用于减少逐行对照 PDF 的学习成本。
...
打造一套以 `ninetrans_book.pdf` 为基础的静态知识点学习系统，让用户不用逐行对照 PDF，也能完整学习每个 setup 的：

1. **识别** setup
2. **拒绝**低质量 setup
3. **执行**入场 / 止损 / 目标
4. **管理**持仓与失败后纪律
5. **追溯**每条规则在 PDF / TXT 中的来源锚点

读图训练、模拟盘交易记录、真实执行统计可以后续拆成独立工具；当前系统优先保证知识点正确、完整、可追溯。
...
导航不设置强制先修门槛。用户可以随时查看任意 setup；掌握度只作为学习反馈，不作为内容访问限制。
...
单元完成用于反馈学习状态，不用于锁定后续内容。完成标准建议同时满足：
...
A2 是第一优先级模板模块。先把 A2 的知识点、来源追溯、案例、题目格式打磨完整；后续 W1P / DP / fBO 按同一模板补齐。A2 不作为访问其它 setup 的硬门槛。
...
1. 可按 setup 自由浏览，不因掌握度或先修关系锁定内容
2. 每个 setup 均有 8 段统一结构
3. 每个 setup 均有 4 类训练题
4. 每题均显示“为什么对 / 为什么错”
5. 进度可持久化（localStorage）
6. A2 课程必须展示结构化 PDF / TXT 来源锚点，后续 setup 按同一模板补齐
